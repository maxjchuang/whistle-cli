...
async function runProxyRollback(
  ...
  const res = await executor.executeRollback(
    ...
    async (handle) => {
      ...
      const h = handle as ProxyRollbackHandle;
  ...
  process.stdout.write(
    renderEnvelope(
      okEnvelope('proxy', 'rollback', res, { instance: resolved, effective: true }),
      format,
    ),
  );
}
  ...
  proxy
    ...
    .action(async () => {
      ...
      try {
        ...
        process.stdout.write(
          renderEnvelope(okEnvelope('proxy', action, st, { instance: resolved }), format),
        );
      } catch (e) {
        const err = CliError.fromUnknown(e);
        process.stderr.write(
          renderEnvelope(errorEnvelope('proxy', action, err, { instance: resolved }), format),
        );
        process.exitCode = 1;
      }
    });
      ...
      async (cmdOpts: {
        ...
        if (cmdOpts.rollback) {
          try {
            ...
            process.stderr.write(
              renderEnvelope(
                errorEnvelope('proxy', 'rollback', err, { instance: resolved }),
                format,
              ),
            );
            process.exitCode = 1;
          ...
          process.stdout.write(
            renderEnvelope(
              okEnvelope(
                'proxy',
                action,
                { ...result, permission: permissionHintForSystemProxySet() },
                {
                  instance: resolved,
  ...
  proxy
    ...
    .action(async () => {
      ...
      try {
        ...
        const st = await service.status(inst.host, inst.port, resolved.id);
        if (!st.active) {
          const envelope = blockedEnvelope('proxy', action, st, {
            instance: resolved,
        ...
        process.stdout.write(
          renderEnvelope(
            okEnvelope('proxy', action, st, { instance: resolved, effective: true }),
            format,
          ),
        );
      } catch (e) {
