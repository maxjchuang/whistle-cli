...
describe('US4 plugins (integration)', () => {
  it('plugins list/install/inspect/enable/disable/uninstall produce envelopes and support rollback', async () => {
    ...
    const empty = await runCli(['--instance', 'dummy', 'plugins', 'list', '--format', 'json'], {
      env,
    });
    ...
    const inspect = await runCli(
      ['--instance', 'dummy', 'plugins', 'inspect', 'whistle.test', '--format', 'json'],
      { env },
    );
    ...
    const disable = await runCli(
      ['--instance', 'dummy', 'plugins', 'disable', 'whistle.test', '--apply', '--format', 'json'],
      { env },
    );
